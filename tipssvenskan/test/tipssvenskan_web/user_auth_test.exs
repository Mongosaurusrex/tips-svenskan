defmodule TipssvenskanWeb.UserAuthTest do
  use TipssvenskanWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Tipssvenskan.Accounts
  alias Tipssvenskan.Accounts.Scope
  alias TipssvenskanWeb.UserAuth

  import Tipssvenskan.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TipssvenskanWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TipssvenskanWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: TipssvenskanWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TipssvenskanWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
